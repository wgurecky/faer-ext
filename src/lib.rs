...
const _: () = {
	...
	impl<'a, T, R: Dim, C: Dim, RStride: Dim, CStride: Dim> IntoFaer for MatrixView<'a, T, R, C, RStride, CStride> {
		...
		fn into_faer(self) -> Self::Faer {
			...
			let ptr = self.as_ptr();
			unsafe { faer::MatRef::from_raw_parts(ptr, nrows, ncols, strides.0.try_into().unwrap(), strides.1.try_into().unwrap()) }
		}
	}
	...
	impl<'a, T, R: Dim, C: Dim, RStride: Dim, CStride: Dim> IntoFaer for MatrixViewMut<'a, T, R, C, RStride, CStride> {
		...
		fn into_faer(self) -> Self::Faer {
			...
			unsafe { faer::MatMut::from_raw_parts_mut(ptr, nrows, ncols, strides.0.try_into().unwrap(), strides.1.try_into().unwrap()) }
		}
	}
	...
	impl<'a, T> IntoNalgebra for MatRef<'a, T> {
		...
		fn into_nalgebra(self) -> Self::Nalgebra {
			...
			let ptr = self.as_ptr();
			unsafe {
				MatrixView::<'_, T, Dyn, Dyn, Dyn, Dyn>::from_data(ViewStorage::<'_, T, Dyn, Dyn, Dyn, Dyn>::from_raw_parts(
					ptr,
					(Dyn(nrows), Dyn(ncols)),
					(Dyn(row_stride.try_into().unwrap()), Dyn(col_stride.try_into().unwrap())),
				))
			}
		}
	}
	...
	impl<'a, T> IntoNalgebra for MatMut<'a, T> {
		...
		fn into_nalgebra(self) -> Self::Nalgebra {
			...
			unsafe {
				MatrixViewMut::<'_, T, Dyn, Dyn, Dyn, Dyn>::from_data(ViewStorageMut::<'_, T, Dyn, Dyn, Dyn, Dyn>::from_raw_parts(
					ptr,
					(Dyn(nrows), Dyn(ncols)),
					(Dyn(row_stride.try_into().unwrap()), Dyn(col_stride.try_into().unwrap())),
				))
...
const _: () = {
	...
	impl<'a, T> IntoNdarray for MatRef<'a, T> {
		...
		fn into_ndarray(self) -> Self::Ndarray {
			...
			let ptr = self.as_ptr();
			unsafe { ArrayView::<'_, T, Ix2>::from_shape_ptr((nrows, ncols).strides((row_stride, col_stride)), ptr) }
		}
	}
	...
	impl<'a, T> IntoNdarray for MatMut<'a, T> {
		...
		fn into_ndarray(self) -> Self::Ndarray {
			...
			let col_stride: usize = self.col_stride().try_into().unwrap();
			let ptr = self.as_ptr_mut();
			unsafe { ArrayViewMut::<'_, T, Ix2>::from_shape_ptr((nrows, ncols).strides((row_stride, col_stride)), ptr) }
		}
	}
};
...
pub mod polars {
	...
	impl Frame for LazyFrame {
		fn is_valid(self) -> PolarsResult<LazyFrame> {
			...
			let test_no_nulls: bool = self
				...
				.with_column(fold_exprs(lit(0).cast(DataType::UInt64), |acc, x| Ok(Some(acc + x)), [col("*")]).alias("sum"))
				.select(&[col("sum")])
				.collect()?
				.column("sum")?
				.u64()?
			...
			match (test_dtypes, test_no_nulls) {
				(true, true) => Ok(self),
				(false, true) => Err(PolarsError::InvalidOperation("frame contains non-numerical data".into())),
				(true, false) => Err(PolarsError::InvalidOperation("frame contains null entries".into())),
				(false, false) => Err(PolarsError::InvalidOperation("frame contains non-numerical data and null entries".into())),
			}
		}
	}
	...
	macro_rules! polars_impl {
		($ty: ident, $dtype: ident, $fn_name: ident) => {
			...
			#[cfg(feature = "polars")]
			#[cfg_attr(docsrs, doc(cfg(feature = "polars")))]
			pub fn $fn_name(frame: impl Frame) -> PolarsResult<Mat<$ty>> {
				use core::iter::zip;
				use core::mem::MaybeUninit;

				fn implementation(lf: LazyFrame) -> PolarsResult<Mat<$ty>> {
					let df = lf.select(&[col("*").cast(DataType::$dtype)]).collect()?;
					...
					df.get_column_names()
						...
						.try_for_each(|(j, col)| -> PolarsResult<()> {
							...
							// `nrows` rows
							let out_col =
								unsafe { core::slice::from_raw_parts_mut(out_ptr.wrapping_add(stride * j) as *mut MaybeUninit<$ty>, nrows) };

							df.column(col)?.$ty()?.downcast_iter().try_for_each(|chunk| -> PolarsResult<()> {
								let len = chunk.len();
								if len == 0 {
									return Ok(());
								}
								...
								match row_start.checked_add(len) {
									Some(next_row_start) => {
										if next_row_start <= nrows {
											...
											match validity {
												Some(bitmap) => {
													...
													if offset > 0 {
														...
														for (out_elem, value_elem) in zip(out_prefix, values_prefix) {
													...
													if bytes.len() > 0 {
														for (out_slice8, values8) in
															zip(out_slice.chunks_exact_mut(BITS_PER_BYTE), values.chunks_exact(BITS_PER_BYTE))
														{
												...
												None => {
													...
													let values = unsafe {
														core::slice::from_raw_parts(values.as_ptr() as *const MaybeUninit<$ty>, values.len())
													};
											...
											Err(PolarsError::ShapeMismatch(format!("too many values in column {col}").into()))
										}
									},
									None => Err(PolarsError::ShapeMismatch(format!("too many values in column {col}").into())),
								}
							})?;

							if row_start < nrows {
								Err(PolarsError::ShapeMismatch(
									format!("not enough values in column {col} (column has {row_start} values, while dataframe has {nrows} rows)")
										.into(),
								))
							} else {
								Ok(())
...
const _: () = {
	use faer::prelude::*;
	use numpy::{Element, PyArrayMethods, PyReadonlyArray1, PyReadonlyArray2};
